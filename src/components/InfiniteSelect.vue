<template>
  ...
  >
    ...
    <template v-slot:append>
      <div class="row items-center">
        <div v-if="multiple" style="padding-top: 3px">
          <div
            v-if="maxCount"
            class="row justify-end text-caption"
          >{{ model.length }}/{{ maxCount }}</div>
    ...
    <template v-slot:after>
      <q-btn v-if="btnAfter" round dense flat icon="add" @click="onBtnAft">
  ...
  methods: {
    ...
    onLoad (page, done) {
      ...
      if (this.filter !== '' && this.filter !== null) {
        ...
        params.description__icontains = this.filter
        params.name__icontains = this.filter
      }
      ...
      this.$axios
        ...
        .then(response => {
          const responseData = response.data[Object.getOwnPropertyNames(response.data)[0]]
          if (responseData) {
